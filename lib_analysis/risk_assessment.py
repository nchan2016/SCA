...
def main() :
    software = argv[1]
    filename = argv[2]
    err = import_json(filename, software)
    if err != 0:
        print(err)
    return 0

def import_json(filename, library):
    f = open(filename, 'r')
    data = json.load(f)
    if_fixed = False
    exploit_availible= False
    for cve in data[library]:
        print(cve["CVE"])
        ...
        Risk = [MI[0] * MF_final, MI[1] * MF_final, MI[2] * MF_final]
        print("MI: ", MI)
        print("Risk: \n")
        print("C: ", Risk[0])
        print("I: ", Risk[1])
...
def find_MI(value_dict):
    return [value_dict["B_C"], value_dict["B_I"], value_dict["B_A"]]
